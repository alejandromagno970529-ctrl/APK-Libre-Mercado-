// lib/main.dart - VERSIÓN COMPLETAMENTE CORREGIDA PARA 16.0.2
...
// Importar servicios de chat corregidos
...
// ✅ MÉTODO: Inicializar notificaciones locales CORREGIDO PARA VERSIÓN 16.0.2
Future<void> _initializeLocalNotifications() async {
  try {
    // ✅ CORRECCIÓN PARA 16.0.2: NO usar const, usar final
    ...
    await flutterLocalNotificationsPlugin.initialize(
      ...
      onDidReceiveNotificationResponse: (NotificationResponse response) {
        ...
        if (response.payload?.startsWith('chat_') ?? false) {
          ...
          // La navegación se manejará en el router según el estado de la app
        }
      },
    );

    // ✅ CORRECCIÓN PARA 16.0.2: AndroidNotificationChannel no es const
...
class _AuthWrapperState extends State<AuthWrapper> {
  ...
  Future<void> _initializeApp() async {
    try {
      ...
      AppLogger.d('📊 Resultado diagnóstico presencia: ${presenceDiagnostic['success']}');
      
      if (presenceDiagnostic['success'] != true) {
        AppLogger.w('⚠️ Sistema de presencia tiene problemas');
        AppLogger.w('💡 Ejecuta este SQL en Supabase SQL Editor:');
        AppLogger.w('''
-- 1. Crear tabla user_presence
...
''');
        ...
        AppLogger.d('✅ Sistema de presencia funciona correctamente');
      }
      
      // ✅ INICIALIZAR SISTEMA DE PRESENCIA PRIMERO
      AppLogger.d('🔧 Inicializando sistema de presencia...');
      final presenceInit = await presenceService.initializeCompleteSetup();
      
      if (!presenceInit['success'] && presenceInit['sql_required'] == true) {
        AppLogger.w('⚠️ Se requiere SQL para sistema de presencia: ${presenceInit['message']}');
        AppLogger.w('📋 SQL recomendado: ${presenceInit['recommended_sql'] ?? 'Consulta los logs'}');
      } else if (presenceInit['success']) {
        AppLogger.d('✅ Sistema de presencia inicializado correctamente');
      }
      ...
      AppLogger.d('   - Presence System: ${presenceInit['success'] ? 'OK' : 'Needs Setup'}');
...
''');
  ...
  static void checkFileServicesState(BuildContext context) {
    final chatProvider = Provider.of<ChatProvider>(context, listen: false);
    
    AppLogger.d(''' 
📁 FILE SERVICES STATE:
   - Chat Provider File Service: ${chatProvider != null}
''');
  }
