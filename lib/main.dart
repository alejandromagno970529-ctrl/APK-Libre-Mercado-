// lib/main.dart - VERSIÓN ACTUALIZADA CON RUTAS DE TIENDA
      ...
      child: MaterialApp(
        ...
        onGenerateRoute: (settings) {
          switch (settings.name) {
            ...
            // ✅ NUEVA RUTA PARA TIENDA DE USUARIO
            case '/user-store':
              final args = settings.arguments as Map<String, dynamic>?;
              if (args != null && args.containsKey('userId')) {
                return MaterialPageRoute(
...
// ✅ CORREGIDO: AuthStateHandler con verificación de sesión mejorada
...
class _AuthStateHandlerState extends State<AuthStateHandler> {
  ...
  Future<void> _checkAuthStatus() async {
    try {
      ...
      print('📊 Sesión Supabase: ${session != null}');
      print('👤 Usuario Supabase: ${user?.email}');
      print('🔄 Estado AuthProvider: ${authProvider.isLoggedIn}');
      
      if (session != null && user != null) {
        print('✅ Sesión activa encontrada - Cargando perfil...');
        await authProvider.loadUserProfile(user.id);
      } else {
        print('🔐 No hay sesión activa - Limpiando estado...');
...
extension ContextExtensions on BuildContext {
  ...
  void goToStore({String? userId, bool isCurrentUser = false}) => NavigationHelper.navigateToStore(this, userId: userId, isCurrentUser: isCurrentUser);
