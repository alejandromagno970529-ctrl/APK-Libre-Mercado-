...
class _ChatListScreenState extends State<ChatListScreen> {
  ...
  Future<void> _loadChats() async {
    ...
    if (authProvider.currentUser != null) {
      // ✅ INTENTO CON MÉTODOS ALTERNATIVOS COMUNES
      try {
        // Primero intentamos con getChats
        if (chatProvider.chats.isEmpty) {
          // Si no hay chats, forzamos una recarga
          await chatProvider.getChats();
        }
      } catch (e) {
        print('Error cargando chats: $e');
        // Si falla, mostramos los chats existentes
      }
    }
  }
  ...
  Future<void> _deleteChat(String chatId, ChatProvider chatProvider) async {
    try {
      // ✅ INTENTO CON MÉTODOS ALTERNATIVOS
      // Si no existe deleteChat, simplemente recargamos la lista
      await _loadChats();
      
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Funcionalidad no disponible temporalmente'),
  ...
  Widget _buildChatList(ChatProvider chatProvider, String userId) {
    // Usamos la lista de chats directamente del provider
