...
class _ChatScreenState extends State<ChatScreen> {
  ...
  Future<void> _initializeChat() async {
    ...
    try {
      ...
      if (authProvider.currentUser != null) {
        ...
        // ✅ CORREGIDO: Actualizar presencia con manejo seguro de errores
        try {
  ...
  // ✅ MÉTODO CORREGIDO: Input area con tamaño optimizado
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Padding(
            padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 6), // ✅ MÁS COMPACTO
            child: Row(
              children: [
                // Botón de emoji - MÁS PEQUEÑO
                Container(
                  width: 38,
                  height: 38,
                  decoration: BoxDecoration(
          ...
          // ✅ AGREGAR ESPACIO INFERIOR PARA MEJOR VISUAL
  ...
  // ✅ MÉTODO MEJORADO: Envío de mensajes con scroll automático Y CORRECCIÓN DE TYPING
  Future<void> _sendMessage() async {
    ...
    // ✅ ✅ CORRECCIÓN CRÍTICA: Remover typing status ANTES de enviar
    if (_isTyping) {
      _isTyping = false;
      _typingProvider.setTypingStatus(
        chatId: widget.chatId,
    ...
    try {
      await chatProvider.sendMessage(
        ...
        fromId: currentUser.id,
        fromName: currentUser.username,
        toUserId: widget.otherUserId, // ✅ RECEIVER ID
        productTitle: 'Producto', // Deberías pasar el título real del producto
      );
  ...
  Widget _buildMessageBubble(Message message, String currentUserId) {
    ...
    // ✅ NUEVO: Filtrar mensajes del sistema "Mensajes leídos" duplicados
    if (message.isSystemMessage && message.text == '📨 Mensajes leídos') {
      ...
      // Solo mostrar si:
      // 1. Es el primer mensaje del sistema de "Mensajes leídos"
      // 2. O el anterior no es el mismo mensaje
      if (index > 0) {
        final previousMessage = messages[index - 1];
        if (previousMessage.isSystemMessage && 
            previousMessage.text == '📨 Mensajes leídos') {
          // No mostrar mensajes consecutivos idénticos
