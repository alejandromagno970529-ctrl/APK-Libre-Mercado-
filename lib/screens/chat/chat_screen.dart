// CONTENIDO COMPLETO - Reemplazar archivo existente
// lib/screens/chat/chat_screen.dart - VERSIÓN COMPLETA CON TYPING INDICATOR
import 'dart:async';
import 'dart:io';
...
import 'package:libre_mercado_final_app/providers/typing_provider.dart'; // ✅ NUEVO
import 'package:libre_mercado_final_app/providers/notification_provider.dart'; // ✅ NUEVO
...
import 'package:libre_mercado_final_app/widgets/typing_indicator.dart'; // ✅ NUEVO
...
class _ChatScreenState extends State<ChatScreen> {
  ...
  bool _isTyping = false; // ✅ NUEVO: Control de typing local
  final Set<String> _pendingMessageIds = {};
  final Map<String, bool> _messagesInProgress = {};

  // ✅ NUEVO: Variables para typing
  Timer? _typingTimer;
  late final TypingProvider _typingProvider;

  // ✅ NUEVO: Stream subscription para acuerdos
  ...
  void initState() {
    ...
    _typingProvider = context.read<TypingProvider>(); // ✅ NUEVO
  ...
  // ✅ NUEVO: Detectar cuando el usuario está escribiendo
  void _detectTyping() {
    if (_isSending) return;
    ...
    // Reset timer para auto-remover typing status
    _typingTimer?.cancel();
  ...
  Future<void> _initializeChat() async {
    ...
    try {
      ...
      // ✅ CORRECCIÓN: Suscripción en tiempo real para acuerdos
      _agreementSubscription = agreementProvider.getAgreementStream(widget.chatId).listen((agreements) {
        if (mounted) {
          ...
          _scrollToBottom(); // ✅ Scroll automático cuando llegan nuevos acuerdos
        }
      });
      ...
      if (authProvider.currentUser != null) {
        ...
        // ✅ NUEVO: Actualizar presencia como "en línea"
  ...
  void dispose() {
    ...
    _focusNode.dispose();
    _agreementSubscription?.cancel(); // ✅ Limpiar suscripción
    _typingTimer?.cancel(); // ✅ Limpiar timer de typing
    
    // ✅ NUEVO: Remover typing status al salir
    if (_isTyping) {
      _typingProvider.setTypingStatus(
        chatId: widget.chatId,
        userId: context.read<AuthProvider>().currentUser?.id ?? '',
  ...
  // ✅ CORRECCIÓN MEJORADA: Scroll automático
                    ...
                    child: TextField(
                      ...
                      onChanged: (text) {
                        if (!_isSending) {
                          ...
                          // ✅ NUEVO: Detectar typing en cada cambio
                          _detectTyping();
                        }
                      },
                    ),
  ...
  // ✅ MÉTODO MEJORADO: Envío de mensajes con scroll automático
  Future<void> _sendMessage() async {
    ...
    // ✅ NUEVO: Remover typing status al enviar
    if (_isTyping) {
      _isTyping = false;
      _typingProvider.setTypingStatus(
        chatId: widget.chatId,
    ...
    try {
      ...
      // ✅ CORRECCIÓN: Scroll inmediato después del envío
      WidgetsBinding.instance.addPostFrameCallback((_) {
        _scrollToBottom();
      });
  ...
  // ✅ MÉTODO MEJORADO: Manejo de mensajes con avatares reales
  Widget _buildMessageBubble(Message message, String currentUserId) {
    ...
    // ✅ CORRECCIÓN: Obtener avatar real del usuario
    final authProvider = context.read<AuthProvider>();
    ...
    if (isTempMessage) {
      ...
      return Opacity(
        ...
        child: ChatBubble(
          ...
          showAvatar: true,
          isTemporary: true,
          userAvatarUrl: userAvatarUrl, // ✅ PASA EL AVATAR REAL
        ),
      );
    }
              ...
              Consumer<NotificationProvider>(
                builder: (context, notificationProvider, child) {
                  return FutureBuilder<Map<String, dynamic>?>(
                    future: notificationProvider.getUserPresence(widget.otherUserId),
                    builder: (context, snapshot) {
                      ...
                      final lastSeen = presence?['last_seen'] != null
                          ? DateTime.parse(presence?['last_seen'])
                          : null;
          ...
          actions: [
            TransactionAgreementButton(
              ...
              onAgreementCreated: (agreement) {
                _scrollToBottom(); // ✅ Scroll cuando se crea acuerdo
              },
            ),
            IconButton(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              return Column(
                                children: [
                                  ...
                                  // ✅ NUEVO: Mostrar typing indicator después del último mensaje
