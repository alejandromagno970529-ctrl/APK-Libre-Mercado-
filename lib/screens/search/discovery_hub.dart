// lib/screens/search/discovery_hub.dart - VERSIÓN CORREGIDA
...
class _DiscoveryHubState extends State<DiscoveryHub> {
  ...
  // ✅ NUEVO: Controlar si el widget está montado
  ...
  // ✅ NUEVO: Método seguro para setState
  ...
  Future<void> _loadDiscoveryData() async {
    try {
      ...
      // ✅ CORREGIDO: Verificar si está montado antes de setState
      if (_isMounted) {
        _safeSetState(() {
          _isLoading = false;
        });
  ...
  Future<void> _loadNearbyProducts() async {
    try {
      ...
      // ✅ CORREGIDO: Solo loguear error si está montado
      if (_isMounted) {
  ...
  Future<void> _loadTrendingData() async {
    try {
      ...
      // ✅ CORREGIDO: Solo loguear si está montado
      if (_isMounted) {
  ...
  // ... (el resto de los métodos de construcción de UI se mantienen igual)
        ...
        Padding(
          ...
          child: SizedBox(
            ...
            child: OutlinedButton(
              onPressed: () => widget.onSearch('Todos'),
  ...
  Color _getCategoryColor(String category) {
    final colors = {
      ...
      'Ropa y Accesorios': Colors.pink,
      'Hogar y Jardín': Colors.green,
      'Deportes': Colors.orange,
      'Electrodomésticos': Colors.purple,
      'Videojuegos': Colors.deepPurple,
      'Alimentos y bebidas': Colors.brown,
  ...
  @override
  void dispose() {
    _isMounted = false; // ✅ IMPORTANTE: Marcar como desmontado
    super.dispose();
  }
}