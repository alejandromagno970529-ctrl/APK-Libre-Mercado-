...
// ✅ CORREGIDO: Eliminado import no utilizado de constants.dart
...
class _ProductDetailScreenState extends State<ProductDetailScreen> {
  ...
  Future<void> _calculateDistance() async {
    try {
      ...
      if (currentLocation['success'] == true) {
        ...
        setState(() {
          _distanceInKm = distance / 1000; // Convertir a kilómetros
        });
      }
    } catch (e) {
  ...
  Future<void> _startChat() async {
    final authProvider = Provider.of<AuthProvider>(context, listen: false);
    // ✅ CORREGIDO: Variable chatProvider eliminada ya que no se usa
      ...
      child: Column(
        ...
        children: [
          ...
          Container(
            padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
            decoration: BoxDecoration(
              // ✅ CORREGIDO: Eliminado .withOpacity deprecated
              color: const Color.fromRGBO(255, 193, 7, 25), // Colors.amber.withOpacity(0.1)
              borderRadius: BorderRadius.circular(8),
              border: Border.all(
                // ✅ CORREGIDO: Eliminado .withOpacity deprecated
                color: const Color.fromRGBO(255, 193, 7, 76), // Colors.amber.withOpacity(0.3)
              ),
            ),
            child: Text(
              widget.product.categorias,
              style: const TextStyle(
                color: Color(0xFFE65100), // Colors.amber[800]
          ...
          Row(
            children: [
              const Icon(Icons.location_on, color: Color(0xFFFFA000)), // Colors.amber[700]
              const SizedBox(width: 8),
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    // ✅ CORREGIDO: Null safety para address
                    Text(
  (widget.product.address ?? '').isNotEmpty ? widget.product.address! : 'Ubicación no disponible',
