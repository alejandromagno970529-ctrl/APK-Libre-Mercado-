// lib/screens/search/product_search_screen.dart - VERSIÓN COMPLETAMENTE ACTUALIZADA
...
class _ProductSearchScreenState extends State<ProductSearchScreen> 
  ...
  // ✅ CORREGIDO: Controladores SEPARADOS para cada pestaña
  ...
  // ✅ CORREGIDO: Estados SEPARADOS para cada pestaña
  ...
  // ✅ CORREGIDO: Estados de UI SEPARADOS
  ...
  bool _isStoresGridView = false; // ✅ CAMBIADO A false PARA VISTA HORIZONTAL
  ...
  void initState() {
    ...
    // ✅ CORREGIDO: Inicializar controladores separados
    _productsSearchController.text = widget.initialQuery;
    _storesSearchController.text = '';
    
    _tabController = TabController(length: 2, vsync: this);
    
    // ✅ CORREGIDO: Listeners SEPARADOS para cada controlador
    ...
    // ✅ CORREGIDO: Inicializar providers
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      print('🎯 INICIANDO PANTALLA DE BÚSQUEDA');
      await _initializeProviders();
      
      // ✅ CARGAR TIENDAS AUTOMÁTICAMENTE AL INICIAR
  ...
  // ✅ MÉTODO CORREGIDO: Inicializar providers
  Future<void> _initializeProviders() async {
    ...
    // ✅ CARGAR TIENDAS SI ESTÁN VACÍAS
    if (storeProvider.stores.isEmpty) {
      await storeProvider.fetchStoresWithRealData(); // ✅ NUEVO MÉTODO
    }
    
    // ✅ INICIALIZAR FILTROS CON DATOS REALES
    _filteredStores = storeProvider.stores;
    _filterProducts(); // Filtrar productos inicialmente
    _filterStores();   // Filtrar tiendas inicialmente
    ...
    print('📊 Productos: ${productProvider.products.length}');
    print('📊 Tiendas: ${storeProvider.stores.length}');
  }

  // ✅ MÉTODO ACTUALIZADO: Cargar tiendas automáticamente con datos reales
  Future<void> _loadAllStoresAutomatically() async {
    if (_storesLoaded && _storesInitialized) return;
    
    print('🔄 CARGANDO TIENDAS CON DATOS REALES Y SINCRONIZADOS...');
    ...
    try {
      ...
      if (!storeProvider.isInitialized) {
        ...
        // ✅ USAR NUEVO MÉTODO CON DATOS REALES
        await storeProvider.fetchStoresWithRealData();
      }
      
      // ✅ ACTUALIZAR FILTROS CON LAS TIENDAS CARGADAS
      _filteredStores = storeProvider.stores;
      
      // ✅ DIAGNÓSTICO MEJORADO CON DATOS REALES
  ...
  // ✅ NUEVO: DIAGNÓSTICO CON DATOS REALES
...
🎯 DIAGNÓSTICO TIENDAS CON DATOS REALES:
   ...
   - Total productos reales: ${storeProvider.stores.fold(0, (sum, store) => sum + store.productCount)}
''');

    final storesWithProducts = storeProvider.stores.where((s) => s.productCount > 0);
    if (storesWithProducts.isNotEmpty) {
      print('🏪 TIENDAS CON PRODUCTOS REALES:');
      for (final store in storesWithProducts.take(3)) {
        print('''
   → ${store.name}:
...
''');
      ...
      print('ℹ️ No hay tiendas con productos reales aún');
    }
  }

  // ✅ MÉTODOS SEPARADOS para cambios en búsqueda de PRODUCTOS
  ...
  // ✅ MÉTODOS SEPARADOS para cambios en búsqueda de TIENDAS
  ...
  // ✅ MÉTODO SEPARADO: Filtrar solo PRODUCTOS
  void _filterProducts() {
    final productProvider = Provider.of<ProductProvider>(context, listen: false);

    print('🔄 FILTRANDO PRODUCTOS - Búsqueda: "${_productsSearchController.text}"');
  ...
  // ✅ MÉTODO SEPARADO: Filtrar solo TIENDAS
  void _filterStores() {
    final storeProvider = Provider.of<StoreProvider>(context, listen: false);

    print('🏪 FILTRANDO TIENDAS - Búsqueda: "${_storesSearchController.text}"');
    ...
    try {
      ...
      print('✅ Tiendas filtradas: ${_filteredStores.length}');
      
    } catch (e) {
      print('❌ Error filtrando tiendas: $e');
      // Fallback: usar todas las tiendas si hay error
      _filteredStores = storeProvider.stores;
    }
  ...
  // ✅ MÉTODOS SEPARADOS para limpiar búsquedas
  ...
  // ✅ MÉTODOS SEPARADOS para limpiar filtros
  ...
  // ✅ MÉTODO SEPARADO: Filtros de PRODUCTOS
  ...
  // ✅ MÉTODO SEPARADO: Filtros de TIENDAS
  ...
  // ✅ MÉTODO: Iniciar chat con vendedor (solo para productos)
  ...
  // ✅ WIDGET SEPARADO: Sección de PRODUCTOS
  ...
  // ✅ WIDGET SEPARADO: Sección de TIENDAS
  Widget _buildStoresSection() {
    final storeProvider = Provider.of<StoreProvider>(context);
    
    print('🏪 CONSTRUYENDO SECCIÓN TIENDAS');
    print('   - Tiendas en provider: ${storeProvider.stores.length}');
    print('   - Tiendas filtradas: ${_filteredStores.length}');
    print('   - Búsqueda actual: "${_storesSearchController.text}"');
    print('   - Vista grid: $_isStoresGridView');

    // ✅ DIAGNÓSTICO DETALLADO
    if (_filteredStores.isNotEmpty) {
      final firstStore = _filteredStores.first;
      print('''
🎯 DATOS DE LA PRIMERA TIENDA:
   - Nombre: ${firstStore.name}
   - Productos REALES: ${firstStore.productCount}
   - Ventas REALES: ${firstStore.totalSales}
   - Rating: ${firstStore.rating}
   - Descripción: ${firstStore.description}
   - Categoría: ${firstStore.category}
''');
    }
        ...
        child: _isStoresGridView
            ...
            : ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  return StoreCard(
                    store: store,
                    isGrid: false, // ← VISTA HORIZONTAL (COMO PROFILE_SCREEN)
                    onTap: () {
                      Navigator.push(
                        context,
                        MaterialPageRoute(
  ...
  // ✅ WIDGETS SEPARADOS para estados vacíos
  ...
  // ✅ WIDGET SEPARADO: Barra de búsqueda de PRODUCTOS
  ...
  // ✅ WIDGET SEPARADO: Barra de búsqueda de TIENDAS
  ...
  // ✅ WIDGET PRINCIPAL con lógica SEPARADA
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        actions: [
          // ✅ El botón de filtros cambia según la pestaña activa
          IconButton(
        ...
        bottom: TabBar(
          ...
          onTap: (index) {
            print('🎯 CAMBIANDO PESTAÑA: ${index == 0 ? 'Productos' : 'Tiendas'}');
            
            // ✅ CARGAR TIENDAS AUTOMÁTICAMENTE AL CAMBIAR A PESTAÑA TIENDAS
            if (index == 1 && (!_storesLoaded || !_storesInitialized)) {
              _loadAllStoresAutomatically();
            }
            
            // ✅ LIMPIAR EL FOCO AL CAMBIAR DE PESTAÑA
            _productsSearchFocusNode.unfocus();
            _storesSearchFocusNode.unfocus();
            setState(() {
              _showProductsRealTimeSuggestions = false;
      ...
      body: SafeArea(
        ...
        child: Column(
          children: [
            // ✅ BARRA DE BÚSQUEDA DINÁMICA según la pestaña activa
