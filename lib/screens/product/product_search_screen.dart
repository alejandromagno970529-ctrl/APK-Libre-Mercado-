// lib/screens/search/product_search_screen.dart - VERSIÓN CORREGIDA SIN DEBUG
...
class _ProductSearchScreenState extends State<ProductSearchScreen> 
  ...
  // ✅ Cargar todas las tiendas automáticamente
  void _loadAllStoresAutomatically() async {
    setState(() {
      _isLoadingStores = true;
    });
    
    final authProvider = Provider.of<AuthProvider>(context, listen: false);
    final allStores = await authProvider.getAllStores();
    ...
    print('✅ Tiendas cargadas automáticamente: ${allStores.length}');
  }
  ...
  Future<void> _filterContent() async {
    ...
    // ✅ CORREGIDO: Cargar tiendas SOLO si hay búsqueda, sino usar las ya cargadas
    if (_searchController.text.isNotEmpty) {
      setState(() {
        _isLoadingStores = true;
      });
      
      _filteredStores = await _searchStores(authProvider, _searchController.text);
      
      setState(() {
        _isLoadingStores = false;
      });
    }
    // Si no hay búsqueda, las tiendas ya están cargadas automáticamente
  ...
  Future<List<AppUser>> _searchStores(AuthProvider authProvider, String query) async {
    try {
      print('🔍 Buscando tiendas con query: "$query"');
      
      final allStores = await authProvider.getAllStores(
        searchQuery: query.isNotEmpty ? query : null,
      );
      
      print('✅ Tiendas encontradas: ${allStores.length}');
      return allStores;
  ...
  void _clearSearch() {
    ...
    // ✅ Recargar todas las tiendas al limpiar búsqueda
    _loadAllStoresAutomatically();
  }
  ...
  // ✅ CORREGIDO: Método para mostrar filtros de tiendas
      ...
      appBar: AppBar(
        ...
        bottom: TabBar(
          ...
          onTap: (index) {
            ...
            if (index == 1 && _filteredStores.isEmpty) {
              _loadAllStoresAutomatically();
            }
          },
          tabs: const [
