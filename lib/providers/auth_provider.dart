...
class AuthProvider with ChangeNotifier {
  ...
  // ✅ CORREGIDO: Pasar _supabase al NotificationService
  // ignore: unused_field
  ...
  // ✅ MÉTODO CORREGIDO: submitVerification con todos los campos requeridos
  Future<void> submitVerification({
    ...
    try {
      ...
      final updateData = {
        ...
        'verification_full_name': fullName,
        'verification_national_id': nationalId,
  ...
  // ✅ MÉTODO NUEVO: updateStoreProfile completo
  Future<String?> updateStoreProfile({
    ...
    try {
      ...
      if (storeCategory != null) updateData['store_category'] = storeCategory;
      ...
      if (storeWebsite != null) updateData['store_website'] = storeWebsite;
      if (storePolicy != null) updateData['store_policy'] = storePolicy;
      ...
      // Si es la primera vez que habilita la tienda, establecer fecha de creación
      ...
      return 'Error actualizando tienda: $e';
    }
  }

  // ✅ MÉTODO NUEVO: clearAuthState para limpiar estado
  ...
  // ✅ MÉTODO CORREGIDO: updateUserPresence - USAR last_active en lugar de last_seen
  Future<void> updateUserPresence(bool isOnline) async {
    try {
      final user = _supabase.auth.currentUser;
      if (user == null) return;

      // ✅ CORREGIDO: Usar last_active que sí existe en la tabla profiles
      await _supabase
          .from('profiles')
          .update({
            'is_online': isOnline,
      ...
      // Actualizar en user_presence (tabla separada) si existe
      try {
        await _supabase
            .from('user_presence')
            .upsert({
        ...
        AppLogger.w('⚠️ Error en user_presence (no crítico): $e');
      }
  ...
  // ✅ MÉTODO NUEVO: refreshUserProfile para actualizar perfil
  ...
  // ✅ MÉTODO EXISTENTE: getAllStores (mantener igual)
    ...
    try {
      ...
      for (final data in response) {
        ...
        final store = AppUser(
          ...
          lastSeen: userData['last_active'] != null  // ✅ CORREGIDO: usar last_active
              ? DateTime.parse(userData['last_active'])
              : null,
        );
  ...
  // ✅ MÉTODO EXISTENTE: debugAuthState (mantener igual)
  ...
  // ✅ MÉTODO EXISTENTE: initialize (mantener igual)
  ...
  // ✅ MÉTODO EXISTENTE: loadUserProfile (mantener igual)
    ...
    try {
      ...
      if (response != null) {
        ...
        _currentUser = AppUser(
          ...
          // ✅ PRESENCIA - CORREGIDO: usar last_active
          isOnline: userData['is_online'] as bool? ?? false,
          lastSeen: userData['last_active'] != null
              ? DateTime.parse(userData['last_active'])
              : DateTime.now(),
        );
  ...
  // ✅ MÉTODO EXISTENTE: _createInitialProfile (mantener igual)
  ...
  // ✅ MÉTODO EXISTENTE: updateUserProfile (mantener igual)
  ...
  // ✅ MÉTODO EXISTENTE: signUp (mantener igual)
  ...
  // ✅ MÉTODO EXISTENTE: signIn (mantener igual)
  ...
  // ✅ MÉTODO EXISTENTE: signOut (mantener igual)
  ...
  // ✅ MÉTODO EXISTENTE: getUserProfile (mantener igual)
  ...
  // ✅ MÉTODO EXISTENTE: clearError (mantener igual)
  void clearError() {
    _error = null;
    notifyListeners();
  }

  // ✅ MÉTODO EXISTENTE: clearPendingEmail (mantener igual)
  void clearPendingEmail() {
    _pendingVerificationEmail = null;
  }

  // ✅ MÉTODO EXISTENTE: debugState (mantener igual)
  void debugState() {
    AppLogger.d('''
🔍 AUTH PROVIDER STATE:
   - isLoading: $isLoading
...
''');
  ...
  // ✅ MÉTODO EXISTENTE: userId (mantener igual)
  String? get userId {
    return _supabase.auth.currentUser?.id;
  }

  // ✅ MÉTODO EXISTENTE: accessToken (mantener igual)
  ...
  // ✅ MÉTODO EXISTENTE: isEmailConfirmed (mantener igual)
  ...
  // ✅ MÉTODO EXISTENTE: isOwnerOf (mantener igual)
  bool isOwnerOf(String resourceUserId) {
    return _supabase.auth.currentUser?.id == resourceUserId;
  }

  // ✅ MÉTODO EXISTENTE: verifyCredentials (mantener igual)
  ...
  // ✅ MÉTODO EXISTENTE: resetPassword (mantener igual)
  ...
  // ✅ MÉTODO EXISTENTE: resendEmailVerification (mantener igual)
