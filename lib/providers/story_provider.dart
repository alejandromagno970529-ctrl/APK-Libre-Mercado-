...
class StoryProvider with ChangeNotifier {
  ...
  Future<String> _uploadEditedImage(Uint8List imageBytes, String userId) async {
    try {
      ...
      return 'https://picsum.photos/400/800?timestamp=$timestamp&user=$userId';
      
    } catch (e) {
      AppLogger.e('Error en _uploadEditedImage', e);
      final timestamp = DateTime.now().millisecondsSinceEpoch;
      return 'https://picsum.photos/400/800?error=$userId&timestamp=$timestamp';
    }
  }
  ...
  Future<Map<String, dynamic>> getStoryStats(String userId) async {
    try {
      ...
      final storiesWithEditing = userStories.where((s) => s.isActiveAndNotExpired).length;
