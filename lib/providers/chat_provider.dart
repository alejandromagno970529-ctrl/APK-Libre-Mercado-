...
class ChatProvider with ChangeNotifier {
  ...
  Future<void> initializeRLS() async {
    try {
      ...
      // Verificar configuración RLS básica primero
    ...
    for (int attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (attempt == maxRetries) {
          ...
          // No rethrow para no bloquear el envío del mensaje
          break;
        }
  ...
  // ✅ MÉTODO MEJORADO: Eliminar mensaje individual con limpieza y sincronización robusta
  Future<void> deleteMessage(String chatId, String messageId, {bool deleteForEveryone = false}) async {
    try {
      ...
      // 1. Obtener mensaje antes de eliminarlo para actualización local
      // ignore: unused_local_variable
      final message = _messages[chatId]?.firstWhere((m) => m.id == messageId);
      
      // 2. Eliminar usando el servicio mejorado
      ...
      if (_messages.containsKey(chatId)) {
        ...
        // 4. Actualizar último mensaje del chat si es necesario
        _updateChatLastMessage(chatId);
  ...
  // ✅ NUEVO MÉTODO: Actualizar último mensaje del chat
  void _updateChatLastMessage(String chatId) {
    if (_chats.containsKey(chatId) && _messages[chatId] != null && _messages[chatId]!.isNotEmpty) {
...
''');
  ...
  // ✅ NUEVO MÉTODO: Diagnosticar problemas de notificaciones
