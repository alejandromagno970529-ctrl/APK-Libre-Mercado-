...
class ChatProvider with ChangeNotifier {
  ...
  void _forceStreamUpdate(String chatId) {
    try {
      if (_messages[chatId] != null) {
        notifyListeners();
        AppLogger.d('🔄 Forzando actualización de UI para chat: $chatId');
      }
    } catch (e) {
      AppLogger.e('❌ Error forzando actualización: $e');
    }
  }
  ...
  // ✅ CORRECCIÓN: Suscripción mejorada a mensajes
    ...
    try {
      ...
      final subscription = _chatService.getMessagesStream(chatId).listen(
        (messages) {
          ...
          try {
            // ✅ FILTRADO MEJORADO: Solo filtrar mensajes temporales pendientes
            final pendingIds = _pendingMessages[chatId] ?? {};
            final filteredMessages = messages.where((message) {
              return !pendingIds.contains(message.tempId); // Solo filtrar por tempId
            }).toList();

            // ✅ ACTUALIZAR SIEMPRE QUE HAYA CAMBIOS
  ...
  // ignore: unused_element
  bool _shouldUpdateMessages(String chatId, List<Message> newMessages) {
