...
class StoreProvider with ChangeNotifier {
  ...
  // ✅ NUEVO MÉTODO: Buscar tiendas con filtros (similar a productos)
...
''');
  ...
  // ✅ NUEVO MÉTODO: Diagnóstico detallado de tiendas
...
🔍 DIAGNÓSTICO DETALLADO STOREPROVIDER:
   ...
   📊 LISTA DE TIENDAS:
   ${_stores.map((s) => '   → ${s.name} (ID: ${s.id}) - Productos: ${s.productCount} - Categoría: ${s.category} - Activa: ${s.isActive}').join('\n')}
   
   🔍 FILTROS APLICADOS:
   - Tiendas filtradas: ${_searchResults.length}
''');
  }
}