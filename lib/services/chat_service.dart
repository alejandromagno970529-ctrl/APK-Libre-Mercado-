...
class ChatService {
  ...
  Future<Message> sendFileMessage({
    ...
    try {
      ...
      // ✅ CORRECCIÓN: Metadata sin DateTime, solo Strings
      final metadata = {
        'file_url': fileUrl,
        'file_name': fileName,
        'file_size': fileSize,
        'mime_type': mimeType,
        'uploaded_at': DateTime.now().toIso8601String(), // ✅ Ya es String
      };
      ...
      final messageData = {
        ...
        'metadata': metadata, // ✅ Enviar como Map, NO como JSON string
  ...
  // ✅ MÉTODO CORREGIDO: Stream con control de duplicados
    ...
    try {
      ...
      if (recentReadReceipts.isEmpty) {
        final readReceiptMessage = {
          ...
          'type': 'system',
          'is_system': true,
          'metadata': { // ✅ Metadata como Map simple
            'read_receipt': true,
            'read_at': DateTime.now().toIso8601String(),
            'reader_id': userId
          },
