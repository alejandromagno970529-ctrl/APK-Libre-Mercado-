// lib/services/notification_service.dart - VERSIÓN COMPLETAMENTE CORREGIDA
...
class NotificationService {
  ...
  // ✅ MÉTODO PRINCIPAL MEJORADO: Envío de notificaciones con manejo robusto de RLS
  Future<void> sendChatNotification({
    ...
    try {
      AppLogger.d('💬 INICIANDO ENVÍO DE NOTIFICACIÓN para: $toUserId');
      ...
      AppLogger.d('✅ NOTIFICACIÓN GUARDADA EXITOSAMENTE - ID: ${result['id']}');
...
- INSERT: Permitir a usuarios autenticados insertar notificaciones
- SELECT: Usuarios solo pueden ver sus propias notificaciones (auth.uid() = user_id)
- UPDATE: Usuarios solo pueden actualizar sus notificaciones
- DELETE: Usuarios solo pueden eliminar sus notificaciones
...
''');
      ...
      // ✅ CORREGIDO: Condición booleana explícita
  ...
  Future<Map<String, dynamic>> checkFullRLSStatus() async {
    try {
      ...
      AppLogger.d('📋 Políticas RLS actuales: ${policies.length}');
  ...
  Future<Map<String, dynamic>> diagnoseNotificationIssues() async {
    try {
      ...
      results['rls_enabled'] = true; // Asumimos que está habilitado si podemos ver políticas
      results['policies_exist'] = policies.isNotEmpty;
      AppLogger.d('3. RLS habilitado: ${results['rls_enabled'] ? '✅' : '❌'}');
      AppLogger.d('4. Políticas existentes: ${policies.length}');

      // 4. Probar operaciones
      final operationsTest = await _testAllRLSOperations();
      results['operations_test'] = operationsTest;

      // ✅ CORREGIDO: Todas las condiciones son booleanas explícitas con manejo de nulos
      final bool authenticationOk = results['authentication'] == true;
      final bool tableExistsOk = results['table_exists'] == true;
      final bool rlsEnabledOk = results['rls_enabled'] == true;
      final bool policiesExistOk = results['policies_exist'] == true;
      ...
      AppLogger.d('   - RLS habilitado: ${rlsEnabledOk ? '✅' : '❌'}');
      AppLogger.d('   - Políticas existentes: ${policiesExistOk ? '✅' : '❌'}');
