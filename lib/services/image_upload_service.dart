...
class ImageUploadService {
  ...
  // ✅ MÉTODO CORREGIDO: uploadChatImage con path simplificado
  Future<String> uploadChatImage(File imageFile, String userId) async {
    try {
      AppLogger.d('🖼️ SUBIENDO IMAGEN PARA CHAT...');
      ...
      // 3. ✅ CORREGIDO: Generar nombre único SIN subcarpetas
      final timestamp = DateTime.now().millisecondsSinceEpoch;
      final fileName = 'chat_image_${timestamp}_${userId.substring(0, 8)}.jpg';
      final filePath = fileName; // ✅ SOLO nombre del archivo

      AppLogger.d('📁 Subiendo imagen: $fileName al bucket chat-images');

      // 4. Leer archivo
      final List<int> imageBytesList = await imageFile.readAsBytes();
      final Uint8List imageBytes = Uint8List.fromList(imageBytesList);

      // 5. ✅ CORREGIDO: Subir directamente al bucket chat-images
      ...
      try {
        ...
        // ✅ MEJOR MANEJO DE ERRORES: Si chat-images falla, usar files
        AppLogger.w('⚠️ Error con bucket chat-images: $e');
        AppLogger.w('🔄 Usando bucket files como fallback...');
        
        bucketName = 'files';
        final fallbackFileName = 'chat_image_${timestamp}_${userId.substring(0, 8)}.jpg';
        await _supabase.storage
            .from(bucketName)
            .uploadBinary(fallbackFileName, imageBytes);
            
        AppLogger.d('✅ Imagen subida exitosamente a files (fallback)');
      }
      ...
      if (e.toString().contains('row-level security policy')) {
        AppLogger.e('🔴 ERROR RLS: Verifica las políticas del bucket chat-images');
  ...
  Future<String?> uploadStoryImage(File imageFile, String userId) async {
    try {
      ...
      final filePath = fileName; // ✅ CORREGIDO: Path simplificado
  ...
  Map<String, String>? _parseImageUrl(String imageUrl) {
    try {
      ...
      for (int i = 0; i < pathSegments.length; i++) {
        ...
        if (segment == 'profile-images' || segment == 'product-images' || segment == 'stories' || segment == 'store-logos' || segment == 'store-banners' || segment == 'chat-images') {
  ...
  // ✅ MÉTODO ADICIONAL: uploadFile para archivos generales
  Future<String> uploadFile(File file, String userId) async {
    try {
      ...
      AppLogger.d('📁 Subiendo: $fileName a files');
      ...
      // 5. SUBIR ARCHIVO AL BUCKET DE ARCHIVOS
      AppLogger.d('🔄 Subiendo a files...');
  ...
  // ✅ MÉTODO COMPATIBILIDAD: uploadImage (alias de uploadProductImage)
  Future<String> uploadImage(File imageFile, String userId) async {
    final result = await uploadProductImage(imageFile, userId);
    if (result == null) {
  ...
  // ✅ NUEVO MÉTODO: Diagnóstico de buckets
  Future<void> diagnoseBuckets() async {
    try {
      AppLogger.d('🔍 INICIANDO DIAGNÓSTICO DE BUCKETS...');
      ...
      final requiredBuckets = ['chat-images', 'files', 'profile-images', 'product-images'];
      ...
      AppLogger.e('❌ Error en diagnóstico de buckets: $e');
    }
  }
}